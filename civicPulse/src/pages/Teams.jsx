...
const Teams = () => {
  ...
  const fetchTeams = async () => {
    try {
      ...
      const transformedTeams = data.map(team => ({
        ...
        createdDate: team.created_at ? new Date(team.created_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
      }));
    ...
    if (newTeam.name.trim() && newTeam.members.some(m => m.trim())) {
      try {
        ...
        const newTeamData = {
          ...
          tasks: [],
          createdDate: data.team.created_at ? new Date(data.team.created_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
        };
  ...
  const handleDeleteTeam = (teamId) => {
    if (window.confirm('Are you sure you want to delete this team?')) {
      // TODO: Add API call to delete team from database
  ...
  if (loading) {
    return (
      <div className="teams-page">
        <Navbar title={`${getDepartmentName()} - Team Management`} subtitle="Manage your teams and assignments" />
        <div className="teams-content" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '400px' }}>
          <p>Loading teams...</p>
        </div>
      </div>
    );
